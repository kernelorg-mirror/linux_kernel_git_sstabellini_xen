...
static int pvcalls_conn_back_write(struct sock_mapping *map)
{
	...
	size = pvcalls_queued(prod, cons, XEN_FLEX_RING_SIZE(map->ring_order));
	if (size == 0)
		return 0;
	...
	if (pvcalls_mask_order(prod, map->ring_order) > pvcalls_mask_order(cons, map->ring_order)) {
		vec[0].iov_base = data->out + pvcalls_mask_order(cons, map->ring_order);
		vec[0].iov_len = size;
		msg.msg_iter.kvec = vec;
		msg.msg_iter.nr_segs = 1;
	} else {
		vec[0].iov_base = data->out + pvcalls_mask_order(cons, map->ring_order);
		vec[0].iov_len = XEN_FLEX_RING_SIZE(map->ring_order) - pvcalls_mask_order(cons, map->ring_order);
