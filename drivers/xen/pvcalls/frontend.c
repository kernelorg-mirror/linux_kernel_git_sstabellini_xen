...
static int pvcalls_front_read_todo(struct sock_mapping *map)
{
	...
	error = intf->in_error;
	mb();
	return (error != 0 || pvcalls_ring_unconsumed(prod, cons, PVCALLS_RING_SIZE(intf->ring_order))) != 0;
}
...
static int pvcalls_front_write_todo(struct sock_mapping *map)
{
	...
	if (error != 0)
		return error;
	return size - pvcalls_ring_unconsumed(prod, cons, size);
}
...
static int pvcalls_front_write_wait(struct sock_mapping *map)
{
	...
	if (error != 0)
		return error;
	return pvcalls_ring_unconsumed(prod, cons, size);
}
