...
struct sock_mapping {
	...
	int irq;
	grant_ref_t ref;
	struct __pvcalls_data_intf *ring;
	void *bytes;
...
struct pvcalls_recycle {
	struct list_head list;
	struct __pvcalls_data_intf *intf;
	grant_ref_t ref;
	void *bytes;
	int evtchn;
};
...
{
	struct __pvcalls_data_intf *intf = map->ring;
	RING_IDX cons, prod;
	int32_t error;
	...
	error = intf->in_error;
	mb();
	return (error != 0 || pvcalls_queued(prod, cons, XEN_FLEX_RING_SIZE(intf->ring_order))) != 0;
}
...
static int pvcalls_front_write_wait(struct sock_mapping *map)
{
	...
	if (error != 0)
		return error;
	return pvcalls_queued(prod, cons, size);
}
				...
				int addr_len, int flags)
{
	...
	if (!list_empty(&priv->pvcalls_front_free_datarings)) {
		...
		spin_unlock(&priv->pvcallss_lock);
		map->ring = (struct __pvcalls_data_intf *) __get_free_page(GFP_KERNEL | __GFP_ZERO);
		if (map->ring == NULL) {
			ret = -ENOMEM;
			goto out_error;
		}
	...
	map->data.out = map->bytes + XEN_FLEX_RING_SIZE(map->ring->ring_order);
...
static int __write_ring(struct __pvcalls_data_intf *intf,
...
static int __read_ring(struct __pvcalls_data_intf *intf, 
		     ...
		     int flags)
{
	...
	if (len > XEN_FLEX_RING_SIZE(map->ring->ring_order))
		len = XEN_FLEX_RING_SIZE(map->ring->ring_order);
			  ...
			  const struct xenbus_device_id *id)
{
	...
	int version, max_page_order, function_calls;
