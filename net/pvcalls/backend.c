...
static int pvcalls_conn_back_write(struct sock_mapping *map)
{
	...
	size = pvcalls_ring_queued(prod, cons, PVCALLS_RING_SIZE(map->ring_order));
	if (size == 0)
		return 0;
...
static void pvcalls_conn_back_read(unsigned long opaque)
{
	...
	struct msghdr msg;
	struct kvec vec[2];
	PVCALLS_RING_IDX cons, prod, size, wanted;
	struct pvcalls_data_intf *intf = map->ring;
	struct pvcalls_data *data = &map->data;
	int ret;
	...
	size = pvcalls_ring_queued(prod, cons, PVCALLS_RING_SIZE(map->ring_order));
	if (size >= PVCALLS_RING_SIZE(map->ring_order))
		return;
	if (skb_queue_empty(&map->sock->sk->sk_receive_queue)) {
		atomic_set(&map->read, 0);
		return;
	}
	wanted = PVCALLS_RING_SIZE(map->ring_order) - size;

	memset(&msg, 0, sizeof(msg));
	msg.msg_iter.type = ITER_KVEC|WRITE;
	msg.msg_iter.count = wanted;
	if (MASK_PVCALLS_IDX(prod, map->ring_order) < MASK_PVCALLS_IDX(cons, map->ring_order)) {
		vec[0].iov_base = data->in + MASK_PVCALLS_IDX(prod, map->ring_order);
		vec[0].iov_len = wanted;
		msg.msg_iter.kvec = vec;
		msg.msg_iter.nr_segs = 1;
	} else {
		vec[0].iov_base = data->in + MASK_PVCALLS_IDX(prod, map->ring_order);
		vec[0].iov_len = PVCALLS_RING_SIZE(map->ring_order) - MASK_PVCALLS_IDX(prod, map->ring_order);
