...
int pvcalls_front_socket(struct socket *sock)
{
	...
	BUG_ON(id >= PVCALLS_NR_REQ_PER_RING);
	if (RING_FULL(&priv->ring) || priv->rsp[id].id != PVCALLS_INVALID_ID) {
		spin_unlock(&priv->pvcallss_lock);
		return -EAGAIN;
	}
	...
	ret = priv->rsp[id].ret;
	priv->rsp[id].id = PVCALLS_INVALID_ID;
			  ...
			  const struct xenbus_device_id *id)
{
	...
	for (i = 0; i < PVCALLS_NR_REQ_PER_RING; i++)
		priv->rsp[i].id = PVCALLS_INVALID_ID;
