...
static int xensock_front_read_todo(struct xensock_ring_intf *intf)
{
	...
	error = intf->in_error;
	mb();
	return (error != 0 || xensock_ring_queued(prod, cons, sizeof(intf->in)) != 0);
}

static int xensock_front_write_todo(struct xensock_ring_intf *intf)
{
	XENSOCK_RING_IDX cons, prod;
	int32_t error;
	...
	if (error != 0)
		return error;
	return sizeof(intf->out) - xensock_ring_queued(prod, cons, sizeof(intf->out));
}

static int xensock_front_write_wait(struct xensock_ring_intf *intf)
{
	XENSOCK_RING_IDX cons, prod;
	int32_t error;
	...
	if (error != 0)
		return error;
	return xensock_ring_queued(prod, cons, sizeof(intf->out));
}
...
static void xensock_front_free_map(struct xensock_front_priv *priv, struct sock_mapping *map)
{
	...
	for (i = 0; i < XENSOCK_DATARING_PAGES; i++)
		gnttab_end_foreign_access(map->ref[i], 0, 0);
				...
				int addr_len, int flags)
{
	...
	map->ring = (struct xensock_ring_intf *) __get_free_pages(GFP_KERNEL | __GFP_ZERO, XENSOCK_DATARING_ORDER);
	if (map->ring == NULL) {
		ret = -ENOMEM;
		goto out_error;
	}
	...
	for (i = 0; i < XENSOCK_DATARING_PAGES; i++) {
		req->u.connect.ref[i] = gnttab_grant_foreign_access(xensock_front_dev->otherend_id, pfn_to_gfn(virt_to_pfn((void*)map->ring) + i), 0);
		map->ref[i] = req->u.connect.ref[i];
	}
...
static int __write_ring(struct xensock_ring_intf *intf, 
		struct iov_iter	*msg_iter, size_t len)
{
	...
	if (size >= sizeof(intf->out))
		return 0;
	if (len > sizeof(intf->out) - size)
		len = sizeof(intf->out) - size;

	masked_prod = MASK_XENSOCK_IDX(prod, intf->out);
	masked_cons = MASK_XENSOCK_IDX(cons, intf->out);

	if (masked_prod < masked_cons) {
		copy_from_iter(intf->out + masked_prod, len, msg_iter);
	} else {
		if (len > sizeof(intf->out) - masked_prod) {
			copy_from_iter(intf->out + masked_prod, sizeof(intf->out) - masked_prod, msg_iter);
			copy_from_iter(intf->out, len - (sizeof(intf->out) - masked_prod), msg_iter);
		} else {
			copy_from_iter(intf->out + masked_prod, len, msg_iter);
		}
	}
				...
				size_t len)
{
	...
	if (flags & (MSG_CONFIRM|MSG_DONTROUTE|MSG_EOR|MSG_OOB) )
		return -EOPNOTSUPP;
	if ((flags & MSG_DONTWAIT) && !xensock_front_write_todo(map->ring))
		return -EAGAIN;

again:
	count++;
	sent = __write_ring(map->ring, &msg->msg_iter, len);
	if (sent > 0) {
		len -= sent;
		tot_sent += sent;
...
static int __read_ring(struct xensock_ring_intf *intf, 
		struct iov_iter	*msg_iter, size_t len, int flags)
{
	XENSOCK_RING_IDX cons, prod, size, masked_prod, masked_cons;
	int32_t error;
	...
	masked_prod = MASK_XENSOCK_IDX(prod, intf->in);
	masked_cons = MASK_XENSOCK_IDX(cons, intf->in);
	...
	if (masked_prod > masked_cons) {
		copy_to_iter(intf->in + masked_cons, len, msg_iter);
	} else {
		if (len > (sizeof(intf->in) - masked_cons)) {
			copy_to_iter(intf->in + masked_cons, sizeof(intf->in) - masked_cons, msg_iter);
			copy_to_iter(intf->in, len - (sizeof(intf->in) - masked_cons), msg_iter);
		} else {
			copy_to_iter(intf->in + masked_cons, len, msg_iter);
		}
	}
		     ...
		     int flags)
{
	...
	if (len > sizeof(map->ring->in))
		len = sizeof(map->ring->in);

	while (!(flags & MSG_DONTWAIT) && !xensock_front_read_todo(map->ring)) {
		if (count < XENSOCK_FRON_MAX_SPIN)
			count++;
		else
			wait_event_interruptible(map->inflight_conn_req, xensock_front_read_todo(map->ring));
	}
	ret = __read_ring(map->ring, &msg->msg_iter, len, flags);
...
int xensock_front_release(struct socket *sock)
{
	...
	if (map->active) {
		while (wait_event_interruptible(map->inflight_conn_req, xensock_front_write_wait(map->ring) <= 0) != 0);
	}
		...
		struct sock_mapping *map, poll_table *wait)
{
	...
	if (xensock_front_write_todo(intf))
		mask |= POLLOUT | POLLWRNORM;
	if (xensock_front_read_todo(intf))
...
int xensock_front_accept(struct socket *sock, struct socket *newsock, int flags)
{
	...
	for (i = 0; i < XENSOCK_DATARING_PAGES; i++) {
		req->u.accept.ref[i] = gnttab_grant_foreign_access(xensock_front_dev->otherend_id, pfn_to_gfn(virt_to_pfn((void*)map->ring) + i), 0);
		map->ref[i] = req->u.accept.ref[i];
	}
