		...
		struct xen_xensock_request *req)
{
	...
	map->bytes = page;
	map->data.in = page;
	map->data.out = page + XENSOCK_RING_SIZE(map->ring_order);
...
static int xensock_back_socket(struct xenbus_device *dev,
		struct xen_xensock_request *req)
{
	struct xensock_back_priv *priv;
	int ret;
...
static int xensock_conn_back_write(struct sock_mapping *map)
{
	...
	struct xensock_data *data = &map->data;
	struct msghdr msg;
	struct kvec vec[2];
	...
	size = xensock_ring_queued(prod, cons, XENSOCK_RING_SIZE(map->ring_order));
	if (size == 0)
		return 0;
	...
	if (MASK_XENSOCK_IDX(prod, map->ring_order) > MASK_XENSOCK_IDX(cons, map->ring_order)) {
		vec[0].iov_base = data->out + MASK_XENSOCK_IDX(cons, map->ring_order);
		vec[0].iov_len = size;
		msg.msg_iter.kvec = vec;
		msg.msg_iter.nr_segs = 1;
	} else {
		vec[0].iov_base = data->out + MASK_XENSOCK_IDX(cons, map->ring_order);
		vec[0].iov_len = XENSOCK_RING_SIZE(map->ring_order) - MASK_XENSOCK_IDX(cons, map->ring_order);
...
static void xensock_conn_back_read(unsigned long opaque)
{
	...
	size = xensock_ring_queued(prod, cons, XENSOCK_RING_SIZE(map->ring_order));
	if (size >= XENSOCK_RING_SIZE(map->ring_order))
		return;
	if (skb_queue_empty(&map->sock->sk->sk_receive_queue)) {
		atomic_set(&map->read, 0);
		return;
	}
	wanted = XENSOCK_RING_SIZE(map->ring_order) - size;

	memset(&msg, 0, sizeof(msg));
	msg.msg_iter.type = ITER_KVEC|WRITE;
	msg.msg_iter.count = wanted;
	if (MASK_XENSOCK_IDX(prod, map->ring_order) < MASK_XENSOCK_IDX(cons, map->ring_order)) {
		vec[0].iov_base = data->in + MASK_XENSOCK_IDX(prod, map->ring_order);
		vec[0].iov_len = wanted;
		msg.msg_iter.kvec = vec;
		msg.msg_iter.nr_segs = 1;
	} else {
		vec[0].iov_base = data->in + MASK_XENSOCK_IDX(prod, map->ring_order);
		vec[0].iov_len = XENSOCK_RING_SIZE(map->ring_order) - MASK_XENSOCK_IDX(prod, map->ring_order);
		...
		struct xen_xensock_request *req)
{
	...
	switch (req->cmd) {
	case XENSOCK_SOCKET:
		ret = xensock_back_socket(dev, req);
		break;
			 ...
			 const struct xenbus_device_id *id)
{
	...
	err = xenbus_printf(XBT_NIL, dev->nodename, "max-dataring-page-order", "%u",
			    MAX_RING_ORDER);
	if (err)
		pr_warn("%s write out 'max-dataring-page-order' failed\n", __func__);
